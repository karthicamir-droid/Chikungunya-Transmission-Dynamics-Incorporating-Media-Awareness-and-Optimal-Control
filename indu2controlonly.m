...
alpha=0.28012; eta=0.54376; beta=0.10259;
tau=0.48077; delta=0.74591;
...
% Initial conditions
Su(1)=90439692; Sa(1)=100000; Eh(1)=60000; Ia(1)=4469; Is(1)=3342;
R(1)=3000; Me(1)=3751030000; Sv(1)=4963813800; Ev(1)=7802; Iv(1)=1300;
...
while( test2 > delta2)
    ...
    for i=1:M
        ...
        m1 = Lambda_h - (b*beta*Iv(i)*Su(i))/N - sigma*Su(i)*Me(i) - mu_h*Su(i) + phi*Sa(i);
        m2 = -((1-eta)*c*beta*Iv(i)*Sa(i))/N + sigma*Su(i)*Me(i) - (phi+mu_h)*Sa(i);
        m3 = ((b*beta*Iv(i)*Su(i))/N + (1-eta)*c*beta*Iv(i)*Sa(i)/N) - (kappa*epsilon+mu_h)*Eh(i);
...
tspan=[0 T];
[Twc, Zwc]=ode15s(@(t,z) model_withcontrol(t,z,u2_fun),tspan,znot);
...
legend('\bf with control (u_1=0,u_2\neq0)', '\bf without control')

% legend('With control (u_1,u_2)','Without control');
title('Optimal control vs No control : India', 'fontsize',15');
...
function dz=model_withoutcontrol(t,z)
    % Parameters
    Lambda_h=908800; Lambda_v=7008000000; mu_h=0.0142; mu_v=0.5;
    mu1=0.001; mu2=0.073; b=0.6; c=0.3; d=0.45; e=0.8;
    sigma=0.34074; phi=0.81425; rho=0.52869;
...
function dz=model_withcontrol(t,z,u2_fun)
    % Parameters
    Lambda_h=908800; Lambda_v=7008000000; mu_h=0.0142; mu_v=0.5;
    mu1=0.001; mu2=0.073; b=0.6; c=0.3; d=0.45; e=0.8;
    sigma=0.34074; phi=0.81425; rho=0.52869;
    ...
    kappa=0.46312; epsilon=0.93459; gamma=0.76959;
    alpha=0.28012; eta=0.54376; beta=0.10259;
    tau=0.48077; delta=0.74591; omega=0.6;
