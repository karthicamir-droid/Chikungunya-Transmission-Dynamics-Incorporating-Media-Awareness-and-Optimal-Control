...
kappa_CI   = [0.4402 0.4861];
epsilon_CI = [0.8708 0.9875];
gamma_CI   = [0.7106 0.8239];
eta_CI     = [0.4934 0.5939];
tau_CI     = [0.4390 0.5225];
delta_CI   = [0.6841 0.8083];

%% Parameter grids
alpha = linspace(0,0.28012,150);   % recovery rate (rows)
beta  = linspace(0,0.10259,150);   % transmission rate (cols)
...
hMap = pcolor(Beta,Alpha,regionMap);
shading flat;
...
[c1,h1] = contour(Beta,Alpha,R0_med,[1 1],'k','LineWidth',2);
...
[c2,h2] = contour(Beta,Alpha,R0_p05,[1 1],'--','Color',[0.3 0.3 0.3],'LineWidth',2.5);
[c3,h3] = contour(Beta,Alpha,R0_p95,[1 1],'--','Color',[0.3 0.3 0.3],'LineWidth',2.5);

xlabel('\bf \beta','FontSize',13);
ylabel('\bf \alpha','FontSize',13);
title('\bf India: Safe (Green) vs Epidemic (Red) Regions','FontSize',14,'FontWeight','bold');
